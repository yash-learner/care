...
care/fixtures/
├── __init__.py
├── base.py              # CareFixtureBase — assembles the per-domain mixins
├── helpers/             # per-domain fixture mixins (one file per domain)
│   ├── __init__.py
│   ├── utils.py         # FixtureError, AttrDict, slugify, phone helpers
│   ├── request.py       # RequestMixin — post/get/patch over APIClient
│   ├── core.py          # orgs, facilities, locations, devices, users, patients, encounters
│   ├── lab.py           # specimen/observation/charge-item/activity definitions + create_lab_test
│   ├── inventory.py     # product knowledge, products, orders, deliveries
│   ├── billing.py       # accounts, charge items, invoices, payments
│   ├── scheduling.py    # schedules, slots, appointments, token queues
│   ├── managing_org.py  # managing-org links, role assignments
│   └── reports.py       # report templates
├── loaders/             # composable load_* orchestrators (compose helpers into data)
│   ├── __init__.py
│   ├── billing.py       # load_billing
│   ├── lab.py           # load_lab_definitions
│   ├── inventory.py     # load_inventory
│   ├── scheduling.py    # load_scheduling
│   └── managing_org.py  # setup_managing_organization
...
| --- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| --- |
| `CareFixtureBase` (in `base.py`) | One Python method per resource, split across per-domain mixins in `helpers/` and assembled by `CareFixtureBase`. Wraps `APIClient`, returns an `AttrDict`. Call methods on the instance (`base.create_invoice(...)`) regardless of which mixin defines them.          | Yes — write your fixtures against this. |
...
> **Adding a resource helper:** add the method to the relevant mixin in
> `helpers/` (or create a new mixin and add it to the `CareFixtureBase` bases in
> `base.py`). Mixins use `self.post` / `self.get` / `self.patch` from
> `RequestMixin` and `self.fake` / `self.user` from `CareFixtureBase`.
